...
def _read_expected_n(folder: Path) -> int:
    audit = folder / "prova_principal" / "final_audit_summary.json"
    if audit.exists():
        try:
...
def _load_status_map(folder: Path, qwidth: int) -> Dict[str, str]:
    ...
    status_path = search_dir / "full_blocks_final_status.json"
    ...
    final_md = search_dir / "full_blocks_final.md"
    qs: Dict[str, str] = {}
    if final_md.exists():
...
def build_final_json(folder: Path) -> Tuple[Path, int, int]:
    """Write final.json and return its path, total questions, and ok questions."""
    search_dir = folder / "prova_principal"
    final_md = search_dir / "full_blocks_final.md"
    ...
    out = search_dir / "final.json"
    out.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    
    total_questions = len(qitems)
    ok_questions = sum(1 for q in qitems if q.get("structure") == "ok")
    
    return out, total_questions, ok_questions

def _looks_like_exam_folder(p: Path) -> bool:
    return (p / "prova_principal" / "full_blocks_final.md").exists()

def run(base: Path) -> None:
    base = base.resolve()
    if base.is_dir() and _looks_like_exam_folder(base):
        out, total, ok = build_final_json(base)
        ok_percent = (ok / total * 100) if total > 0 else 0
        print(f"[final-json] {base.name}: {ok}/{total} ({ok_percent:.1f}%) questions ok | wrote {out.name}")
        return

    if base.is_dir():
        folders = [f for f in sorted(base.iterdir()) if f.is_dir() and _looks_like_exam_folder(f)]
        if not folders:
            print(f"⚠️ [final-json] No folders with 'prova_principal/full_blocks_final.md' under {base}")
            return
        ...
        for f in folders:
            out, total, ok = build_final_json(f)
            grand_total += total
            grand_ok += ok
            ok_percent = (ok / total * 100) if total > 0 else 0
            print(f"[final-json] {f.name}: {ok}/{total} ({ok_percent:.1f}%) questions ok | wrote {out.name}")
        
        grand_percent = (grand_ok / grand_total * 100) if grand_total > 0 else 0
        print(f"[done] final-json processed {len(folders)} folder(s) | Total ok: {grand_ok}/{grand_total} ({grand_percent:.1f}%)")
        return
