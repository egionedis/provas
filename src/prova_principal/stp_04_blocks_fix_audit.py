...
def _source_file(folder: Path) -> Optional[Path]:
    """
    Use **full_blocks_fix_missing.md** as the reference inside the 'prova_principal' subfolder.
    Skip folder if it doesn't exist (no fallbacks here).
    """
    # --- CHANGE IS HERE ---
    search_dir = folder / "prova_principal"
    p = search_dir / "full_blocks_missing.md"
...
def final_audit_batch(base: Path) -> int:
    ...
    def _run_one(folder: Path) -> int:
        ...
        if not src:
            print(f"⚠️  Skipping {folder.name}: full_blocks_missing.md not found in 'prova_principal' subfolder.")
            return 0

        stats = _audit_file(src)

        # --- CHANGE IS HERE ---
        # Save output to the 'prova_principal' subfolder
        output_dir = folder / "prova_principal"
        output_dir.mkdir(exist_ok=True)
        # --- END OF CHANGE ---

        (output_dir / "final_audit_summary.json").write_text(
        ...
        (output_dir / "final_audit_summary.md").write_text("\n".join(md_lines), encoding="utf-8")
    ...
    if total == 0:
        print("⚠️  No folders with full_blocks_missing.md were found under", base)
        return 1
