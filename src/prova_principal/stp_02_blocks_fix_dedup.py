...
def fix_one(folder: Path) -> bool:
    ...
    if not md_in:
        print(f"⚠️  Skipping {folder.name}: full_blocks.md not found in 'prova_principal' subfolder.")
        return False

    text = md_in.read_text(encoding="utf-8", errors="ignore")
    blocks = _split_blocks(text)
    if not blocks:
        print(f"⚠️  Skipping {folder.name}: no '## Questão N' blocks found in {md_in.name}")
        return False
    ...
    # --- THIS IS THE FIX ---
    # Save the output files to the same 'prova_principal' directory as the input.
    output_dir = folder / "prova_principal"
    md_out = output_dir / "full_blocks_dedup.md"
    # --- END OF FIX ---
    ...
    summary_path = output_dir / "full_blocks_dedup_summary.md"
    summary_path.write_text("\n".join(summary_lines) + "\n", encoding="utf-8")

    # This print statement now shows the full path for clarity
    print(f"[fix] {folder.name}: in={len(blocks)} blocks → out={len(out_blocks)} | deduped={deduped} | instr_detected={instr_detected}")
    return True
