...
def _read_blocks_file(folder: Path) -> tuple[Path, str]:
    # --- CHANGE IS HERE ---
    # Look for the input file inside the 'prova_principal' subfolder.
    search_dir = folder / "prova_principal"
    # Only look for the output from the previous step.
    p = search_dir / "full_blocks_dedup.md"
    if p.exists():
        return p, p.read_text(encoding="utf-8", errors="ignore")
    raise FileNotFoundError("full_blocks_dedup.md not found")
...
def fix_missing_one(folder: Path) -> bool:
    try:
        ...
        print(f"⚠️  Skipping {folder.name}: full_blocks_dedup.md not found in 'prova_principal' subfolder.")
        return False

    blocks = _split_blocks(text)
    if not blocks:
        print(f"⚠️  Skipping {folder.name}: no '## Questão N' blocks in {md_in_path.name}")
        return False

    fixed_blocks, summary_lines = _fix_missing_in_blocks(blocks)

    output_dir = folder / "prova_principal"
    out_md = output_dir / "full_blocks_missing.md"
    out_sum = output_dir / "full_blocks_missing_summary.md"
    ...
    filled_count = sum(1 for line in summary_lines if line.strip().startswith("- Filled Q"))
    
    details = []
    if filled_count > 0:
        details.append(f"filled={filled_count}")

    # Find the line with unresolved issues to report them
    unresolved_line = next((line for line in summary_lines if line.strip().startswith("- Unresolved missing:")), None)
    if unresolved_line:
        unresolved_numbers = unresolved_line.split(":")[-1].strip()
        details.append(f"unresolved=[{unresolved_numbers}]")

    details_str = " | ".join(details) if details else "No gaps found"

    print(f"[fix-missing] {folder.name}: in={len(blocks)} → out={len(fixed_blocks)} | {details_str} | wrote {out_md.name}")
